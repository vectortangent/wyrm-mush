module AresMUSH
  module WOD20Skills
    class WOD20SkillsConfigValidator
      attr_accessor :validator

      def initialize
        @validator = Manage::ConfigValidator.new("wod20skills")
      end
      ...
      def validate
        ...
        begin
          ...
          @validator.add_error "Unknown WOD20Skills config error.  Fix other errors first and try again. #{ex} #{ex.backtrace[0, 3]}"
        end
      ...
      def check_attributes
        abilities = Global.read_config('wod20skills', 'attributes')
        abilities.each do |s|
          name = s['name'] || "Missing Name"
          ['name', 'desc'].each do |prop|
            verify_property_exists(name, s, prop)
          end
          if (WOD20Skills.check_ability_name(name))
            @validator.add_error "wod20skills:attributes #{name} cannot contain special characters."
          end
        end
        if (!WOD20Skills.attr_names.include?(Global.read_config('wod20skills', 'default_linked_attr')))
          @validator.add_error "wod20skills:default_linked_attr is not a valid attribute."
        end
      end

      def check_action_skills
        action_skills = Global.read_config('wod20skills', 'action_skills')
        action_skills.each do |s|
          ...
          if (WOD20Skills.check_ability_name(name))
            @validator.add_error "wod20skills:action_skills #{name} cannot contain special characters."
          end
          (s['specialties'] || []).each do |spec|
            if (WOD20Skills.check_ability_name(spec))
              @validator.add_error "wod20skills:action_skills #{spec} cannot contain special characters."
            end
          end
          if (!WOD20Skills.attr_names.include?(s['linked_attr']))
            @validator.add_error "wod20skills:action_skills #{spec} cannot contain special characters."
          end
        end
      end
      ...
      def check_advantages
        abilities = Global.read_config('wod20skills', 'advantages')
        abilities.each do |s|
          name = s['name'] || "Missing Name"
          ['name', 'desc'].each do |prop|
            verify_property_exists(name, s, prop)
          ...
          if (WOD20Skills.check_ability_name(name))
            @validator.add_error "wod20skills:advantages #{name} cannot contain special characters."
          end
        end
      end

      def check_background_skills
        abilities = Global.read_config('wod20skills', 'background_skills')
        abilities.each do |name, desc|
          if (WOD20Skills.check_ability_name(name))
            @validator.add_error "wod20skills:background_skills #{name} cannot contain special characters."
          end
        end
      end

      def check_languages
        abilities = Global.read_config('wod20skills', 'languages')
        abilities.each do |s|
          name = s['name'] || "Missing Name"
          verify_property_exists(name, s, 'name')
          if (WOD20Skills.check_ability_name(name))
            @validator.add_error "wod20skills:languages #{s['name']} cannot contain special characters."
          end
        end
      end

      def check_xp
        xp = Global.read_config('wod20skills', 'xp_costs')
        categories = ['language', 'action', 'background', 'attribute', 'advantage']
        categories.each do |cat|
          if (!xp.keys.include?(cat))
            @validator.add_error "wod20skills:xp_costs is missing category #{cat}."
          end
        end

        xp.each do |type, data|
          if (!categories.include?(type))
            @validator.add_error "wod20skills:xp_costs #{type} is not a valid ability category."
          end

          data.each do |level, cost|
            if (level.to_i < 0 || level.to_i > 7)
              @validator.add_error "wod20skills:xp_costs #{type} level #{level} is not a valid level."
            end
            if (level.to_i == 0)
              @validator.add_error "wod20skills:xp_costs #{type} level 0->1 always costs 1XP no matter what you put here."
            end
          end
      ...
      def check_misc
        luck_for_scene = Global.read_config('wod20skills', 'luck_for_scene')
        luck_for_scene.each do |scenes, luck|
          if (scenes.to_i < 0)
            @validator.add_error "wod20skills:luck_for_scene #{scenes} is not a valid number of scenes."
          end
          if (!(luck.kind_of?(Integer) || luck.kind_of?(Float)))
            @validator.add_error "wod20skills:luck_for_scene #{luck} is not a valid number of luck points."
          end
        end
        ...
        max_attrs.each do |level, num|
          if (level.to_i == 0)
            @validator.add_error "wod20skills:max_attrs_at_or_above #{level} is not a valid number."
          end

          if (level.to_i < 1 || level.to_i > 4)
            @validator.add_error "wod20skills:max_attrs_at_or_above #{level} must be from 1-4."
          end

          if (num.to_i < 0)
            @validator.add_error "wod20skills:max_attrs_at_or_above #{num} must be greater than 0."
          end
        end
        ...
        max_skills.each do |level, num|
          if (level.to_i == 0)
            @validator.add_error "wod20skills:max_skills_at_or_above #{level} is not a valid number."
          end

          if (level.to_i < 1 || level.to_i > 8)
            @validator.add_error "wod20skills:max_skills_at_or_above #{level} must be from 1-8."
          end

          if (num.to_i < 0)
            @validator.add_error "wod20skills:max_skills_at_or_above #{num} must be greater than 0."
          end
        end

        start_skills = Global.read_config('wod20skills', 'starting_skills')
        start_skills.each do |name, data|
          if (name == "Everyone")
            ...
            if (!group)
              @validator.add_error "wod20skills:starting_skills #{name} is not a valid group."
            end
      ...
      def validate_starting_skills_list(skills, name)
        if (!skills || !skills.kind_of?(Hash))
          @validator.add_error "wod20skills:starting_skills #{name} is missing a skills list."
        end

        skills.each do |skill, rating|
          if (WOD20Skills.check_ability_name(skill))
            @validator.add_error "wod20skills:starting_skills #{skill} is an invalid skill name."
          end
          min_rating = Global.read_config("wod20skills", "allow_incapable_action_skills") ? 0 : 1
          if (!rating.kind_of?(Integer) || rating.to_i < min_rating || rating.to_i > 8)
            @validator.add_error "wod20skills:starting_skills #{skill} has an invalid rating."
          end
        end
      end
      ...
      def verify_property_exists(name, data, prop)
        if (!data[prop])
          @validator.add_error "wod20skills #{name} missing #{prop}."
        end
      end
