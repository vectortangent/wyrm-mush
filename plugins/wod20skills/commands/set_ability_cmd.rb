  ...
  module WOD20Skills
    class SetAbilityCmd
      ...
      def check_valid_rating
        ...
        return t('wod20skills.invalid_rating') if !self.rating.is_integer?
        return nil
      end
      ...
      def handle
        ClassTargetFinder.with_a_character(self.name, client, enactor) do |model|
          ...
          error = WOD20Skills.check_rating(self.ability_name, new_rating)
          if (error)
            client.emit_failure error
            return
          end

          error = WOD20Skills.set_ability(model, self.ability_name, new_rating)
          if (error)
            client.emit_failure error
          else
            client.emit_success WOD20Skills.ability_raised_text(model, self.ability_name)
          end
