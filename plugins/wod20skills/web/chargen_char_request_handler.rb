  ...
  module WOD20Skills
    class ChargenCharRequestHandler
      def handle(request)
        ...
        {
          wod20_attributes: get_ability_list(char, char.wod20_attributes, :attribute),
          wod20_action_skills: get_ability_list(char, char.wod20_action_skills, :action),
          wod20_backgrounds: get_ability_list(char, char.wod20_background_skills, :background),
          wod20_languages: get_ability_list(char, char.wod20_languages, :language),
          wod20_advantages: get_ability_list(char, char.wod20_advantages, :advantage),
          reset_needed: !char.wod20_attributes.map { |a| a.rating > 1 }.any?,
      ...
      def get_ability_list(char, list, ability_type)
        case ability_type
        when :attribute
          metadata = WOD20Skills.attrs
          starting_rating = 1
          starting_rating_name = t('wod20skills.poor_rating')
        when :action
          metadata = WOD20Skills.action_skills
          starting_rating = 1
          starting_rating_name = t('wod20skills.everyman_rating')
        when :language
          metadata = WOD20Skills.languages
          starting_rating = 0
          starting_rating_name = t('wod20skills.everyman_rating')
        when :advantage
          metadata = WOD20Skills.advantages
          starting_rating = 0
          starting_rating_name = t('wod20skills.everyman_rating')
        else
          metadata = nil
        ...
        abilities = list.to_a.sort_by { |a| a.name }.map { |a|
          {
            ...
            desc: (metadata) ? WOD20Skills.get_ability_desc(metadata, a.name) : nil,
