module AresMUSH
  module WOD20Skills
    ...
    def self.roll_ability(char, roll_params)
      dice = WOD20Skills.dice_to_roll_for_ability(char, roll_params)
      roll = WOD20Skills.roll_dice(dice)
      Global.logger.info "#{char.name} rolling #{roll_params} dice=#{dice} result=#{roll}"
      Achievements.award_achievement(char, "wod20_roll")
      roll
    end



    # Rolls a number of WOD20 dice and returns the raw die results.
    ...
    def self.get_success_level(die_result)
      successes = die_result.count { |d| d >= WOD20Skills.success_target_number }
    ...
    def self.emit_results(message, client, room, is_private)
      if (is_private)
        ...
        room.emit message
        channel = Global.read_config("wod20skills", "roll_channel")
        if (channel)
      ...
      Global.logger.info "WOD20 roll results: #{message}"
    end
    ...
    def self.determine_web_roll_result(request, enactor)
      ...
      if (!vs_roll1.blank?)
        ...
        die_result1 = WOD20Skills.parse_and_roll(model1, vs_roll1)
        die_result2 = WOD20Skills.parse_and_roll(model2, vs_roll2)

        if (!die_result1 || !die_result2)
          return { error: t('wod20skills.unknown_roll_params') }
        end

        successes1 = WOD20Skills.get_success_level(die_result1)
        successes2 = WOD20Skills.get_success_level(die_result2)

        results = WOD20Skills.opposed_result_title(vs_name1, successes1, vs_name2, successes2)

        message = t('wod20skills.opposed_roll_result',
           :name1 => !model1 ? t('wod20skills.npc', :name => vs_name1) : model1.name,
           :name2 => !model2 ? t('wod20skills.npc', :name => vs_name2) : model2.name,
           :roll1 => vs_roll1,
           :roll2 => vs_roll2,
           :dice1 => WOD20Skills.print_dice(die_result1),
           :dice2 => WOD20Skills.print_dice(die_result2),
           :result => results)
      ...
      # ------------------
        ...
        roll = WOD20Skills.parse_and_roll(char, pc_skill)
        ...
        message = t('wod20skills.simple_roll_result',
          ...
          :roll => pc_skill,
          :dice => WOD20Skills.print_dice(roll),
      ...
      # ------------------
        ...
        roll = WOD20Skills.parse_and_roll(enactor, roll_str)
        roll_result = WOD20Skills.get_success_level(roll)
        success_title = WOD20Skills.get_success_title(roll_result)
        message = t('wod20skills.simple_roll_result',
          :name => enactor.name,
