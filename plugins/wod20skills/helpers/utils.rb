module AresMUSH
  module WOD20Skills
    ...
    def self.can_view_sheets?(actor)
      return true if Global.read_config("wod20skills", "public_sheets")
    ...
    def self.attrs
      Global.read_config("wod20skills", "attributes")
    end

    def self.action_skills
      Global.read_config("wod20skills", "action_skills")
    end

    def self.languages
      Global.read_config("wod20skills", "languages")
    end

    def self.advantages
      return {} if !WOD20Skills.use_advantages?
      Global.read_config("wod20skills", "advantages")
    end
    ...
    def self.background_skills
      Global.read_config("wod20skills", "background_skills")
    end
    ...
    def self.action_skill_config(name)
      config = WOD20Skills.action_skills.find { |s| s["name"].upcase == name.upcase }
      if (!config)
    ...
    def self.attr_blurb
      Global.read_config("wod20skills", "attributes_blurb")
    end

    def self.action_blurb
      Global.read_config("wod20skills", "action_skills_blurb")
    end

    def self.bg_blurb
      Global.read_config("wod20skills", "bg_skills_blurb")
    end

    def self.language_blurb
      Global.read_config("wod20skills", "language_blurb")
    end

    def self.advantages_blurb
      Global.read_config("wod20skills", "advantages_blurb")
    end
