module AresMUSH
  module WOD20Skills
    module StartingSkills
      def self.config
        Global.read_config("wod20skills", "starting_skills") || {}
      end

      def self.get_skills_for_char(char)
        skills = {}

        WOD20Skills.attrs.map { |a| a['name'] }.each do |a|
          skills[a] = 1
        end

        WOD20Skills.action_skills.map { |a| a['name'] }.each do |a|
          skills[a] = Global.read_config('wod20skills', 'allow_incapable_action_skills') ? 0 : 1
        end
