    ...
    describe WOD20Skills do
      ...
      describe :get_skills_for_char do
        before do
          ...
          allow(Global).to receive(:read_config).with("wod20skills", "starting_skills") { starting_skills }
          allow(Global).to receive(:read_config).with("wod20skills", "attributes") { [ ] }
          allow(Global).to receive(:read_config).with("wod20skills", "action_skills") { [ ] }
        ...
        it "should include default attributes" do
          allow(@char).to receive(:group).with("Position") { "Other" }
          allow(@char).to receive(:group).with("Faction") { "Other" }
          allow(Global).to receive(:read_config).with("wod20skills", "attributes") { [ { 'name' => 'Brawn' }, { 'name' => 'Presence' } ] }
        ...
        it "should include default action skills when incapable allowed" do
          allow(@char).to receive(:group).with("Position") { "Other" }
          allow(@char).to receive(:group).with("Faction") { "Other" }
          allow(Global).to receive(:read_config).with("wod20skills", "action_skills") { [ { 'name' => 'Melee' } ] }
          allow(Global).to receive(:read_config).with("wod20skills", "allow_incapable_action_skills") { true }
