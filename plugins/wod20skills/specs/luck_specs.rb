  ...
  module WOD20Skills
    describe WOD20Skills do
      ...
      describe :award_luck do
        before do
          @char = Character.new(wod20_luck: 1)
        end

        it "should add luck" do
          expect(@char).to receive(:update).with(wod20_luck: 2.0) {}
          @char.award_luck(1)
        end

        it "should not go over the cap" do
          expect(@char).to receive(:update).with(wod20_luck: 3.0) {}
          @char.award_luck(5)
        end
      end
      ...
      describe :spend_luck do
        before do
          @char = Character.new(wod20_luck: 2)
        end

        it "should spend luck" do
          expect(@char).to receive(:update).with(wod20_luck: 1.0)
          @char.spend_luck(1)
        end

        it "should not go below zero" do
          expect(@char).to receive(:update).with(wod20_luck: 0)
          @char.spend_luck(5)
        end
      end
      ...
      describe :luck_for_scene do
        before do
          @char = Character.new(wod20_luck: 2, wod20_scene_luck: {})
          @scene = double
          @base_luck = 0.1
          allow(Global).to receive(:read_config).with("wod20skills", "luck_for_scene") { {
            '0' => @base_luck,
            '10' => @base_luck * 0.75,
            '25' => @base_luck * 0.5
          } }
        ...
        it "should give a bonus for RPing with a newbie" do
          ...
          expect(@char).to receive(:update).with( { :wod20_scene_luck => { 111 => 1 } } )
          WOD20Skills.luck_for_scene(@char, @scene)
        end
        ...
        it "should award regular luck if <10 scenes with the person" do
          ...
          allow(oldbie).to receive(:created_at) { Time.now - 86400*90 }
          allow(oldbie).to receive(:id) { 111 }
          @char.wod20_scene_luck = { 111 => 9 }
          expect(@scene).to receive(:participants) { [oldbie] }
          expect(@char).to receive(:award_luck).with( @base_luck )
          expect(@char).to receive(:update).with( { :wod20_scene_luck => { 111 => 10 } } )
          WOD20Skills.luck_for_scene(@char, @scene)
        end
        ...
        it "should reduce luck if more than 10 scenes" do
          ...
          allow(oldbie).to receive(:created_at) { Time.now - 86400*90 }
          allow(oldbie).to receive(:id) { 111 }
          @char.wod20_scene_luck = { 111 => 11 }
          expect(@scene).to receive(:participants) { [oldbie] }
          expect(@char).to receive(:award_luck).with( @base_luck * 0.75 )
          expect(@char).to receive(:update).with( { :wod20_scene_luck => { 111 => 12 } } )
          WOD20Skills.luck_for_scene(@char, @scene)
        end
        ...
        it "should reduce luck if more than 25 scenes" do
          ...
          allow(oldbie).to receive(:created_at) { Time.now - 86400*90 }
          allow(oldbie).to receive(:id) { 111 }
          @char.wod20_scene_luck = { 111 => 26 }
          expect(@scene).to receive(:participants) { [oldbie] }
          expect(@char).to receive(:award_luck).with( @base_luck * 0.5 )
          expect(@char).to receive(:update).with( { :wod20_scene_luck => { 111 => 27 } } )
          WOD20Skills.luck_for_scene(@char, @scene)
        end
